...
Weights w_ij are scalars assigned to each edge (i,j) used to express an interior vertex (or point) as an affine/barycentric combination of its neighbors:
p_i = sum_{j in N(i)} sigma_ij * p_j, with sigma_ij = w_ij / sum_k w_ik (normalization
{% endalert %}
...
For an interior edge (i \leftrightarrow j):

[
w_{ij} = \frac{1}{2} \left( \cot \alpha_{ij} + \cot \beta_{ij} \right),
]

where (\alpha_{ij}) and (\beta_{ij}) are the angles opposite the edge (ij) in the two adjacent triangles.
...
Consider a convex polygon with vertices (p_1, p_2, ..., p_n).
For each vertex (p_j), the **Wachspress weight** at interior point (p) is:
...
* (A_j) = signed area of triangle ((p, p_j, p_{j+1}))
* (B_j) = area of the quadrilateral formed by edges meeting at vertex (j)

These areas appear in formula:

[
w_j = \frac{B_j}{A_{i-1}A_j}
]
...
To prove Wachspress coordinates reproduce affine functions, he expresses the vector (p) as a combination of neighbors using areas.
...
Where (\theta_j) is the angle between edges to (p_j) and (p_{j+1}).
...
This is necessary to avoid the trivial solution (p_i = \text{constant}).
...
* Measure distortion at every triangle using singular values ( \alpha_1, \alpha_2 ).
